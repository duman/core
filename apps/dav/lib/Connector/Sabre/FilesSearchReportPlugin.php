...
class FilesSearchReportPlugin extends ServerPlugin {
	...
	public function onReport($reportName, $report, $uri) {
		...
		$xml = $this->server->generateMultiStatus($this->getSearchResultIterator($filesUri, $searchResults, $requestedProps));
	 ...
	 * @return \Generator a generator to traverse over the properties of the search result, suitable for server's multistatus response
	 */
	private function getSearchResultIterator($filesUri, $searchResults, $requestedProps) {
		...
		foreach ($nodes as $node) {
			$propFind = new PropFind($filesUri . $node->getPath(), $requestedProps, 0, $propFindType);
